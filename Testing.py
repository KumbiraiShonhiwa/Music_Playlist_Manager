from database import Database, Song, Playlist, User
import pytest
import json
...
class Test:
    ...
    def test_add_song_functionality(self):
        ...
        assert self.playlist1.add_song(
            self.song2.name, self.song2.singer, self.song2.genre, self.song2.runtime) == True
        assert self.playlist1.add_song(
            self.song1.name, self.song1.singer, self.song1.genre, self.song1.runtime) == True
        assert self.playlist1.add_song(
            self.song3.name, self.song3.singer, self.song3.genre, self.song3.runtime) == True
